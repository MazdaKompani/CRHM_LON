...
for i=1:numel(model_tests(:,1))
 ...
 %%%%%   hold on
   ...
   %%%%% hold on
    ...
    CHRMout_data_interpp = interp1(datenum(QSurf_crhm_i.Time),QSurf_crhm_i.Data,datenum(QSurf.Time));
% remove NaNs
isnanLoc = isnan(CHRMout_data_interpp);
CHRMout_data_interpp(isnanLoc) = [];
dataUsep = QSurf.data;
dataUsep(isnanLoc) = [];
for iii=1:numel(dataUsep(:,1))
    plot(CHRMout_data_interpp(iii),dataUsep(iii),'ro','Markersize',2)
    hold on
end
 xlabel('Simulated surface flow(mm/hr)')
 ylabel('Observed surface flow (mm/hr)')
    
   % Performence calculation
     [Nash,RMSE,Bias] = PerformenceCalculator(QSurf_crhm_i.Time,QSurf_crhm_i.Data,...
         QSurf.Time,QSurf.Data);   %chnage QSurf   to QTile
